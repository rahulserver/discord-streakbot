...
let processCommand = async (msg, params, tiers) => {
  try {
    ...
    await DB_UTILS.setField(msg.author.id, "last_cold_shower", new Date());
    let score = await DB_UTILS.getField(msg.author.id, "total_score");
    msg.reply(SCORE + " point added successfully for today's cold shower!\nYour score is now "+score);
  } catch (e) {
...
let commandValidator = async (msg, params, tiers) => {
  ...
  try {
    let last_cold_shower = await DB_UTILS.getField(msg.author.id, "last_cold_shower");
    if (last_cold_shower) {
      let UTC_DATE = last_cold_shower.getUTCDate();
      let UTC_DATE_CUR = new Date().getUTCDate();
      if (UTC_DATE === UTC_DATE_CUR) {
        // command invalid because two commands in same day not allowed
        throw new Error("You have already gained cold shower points for today. Wait for next day(in UTC)");
      }
    }
    return true;
  } catch (e) {
