...
let processCommand = async (msg, params, tiers) => {
  try {
    ...
    await DB_UTILS.setField(msg.author.id, "last_growth", new Date());
    let score = await DB_UTILS.getField(msg.author.id, "total_score");
    msg.reply(SCORE + " point added successfully for today's growth!\nYour score is now "+score);
  } catch (e) {
...
let commandValidator = async (msg, params, tiers) => {
  ...
  try {
    let last_growth = await DB_UTILS.getField(msg.author.id, "last_growth");
    if (last_growth) {
      let UTC_DATE = last_growth.getUTCDate();
      let UTC_DATE_CUR = new Date().getUTCDate();
      if (UTC_DATE === UTC_DATE_CUR) {
        // command invalid because two commands in same day not allowed
        throw new Error("You have already gained growth points for today. Wait for next day(in UTC)");
      }
    }
    return true;
  } catch (e) {
