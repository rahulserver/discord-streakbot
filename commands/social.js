...
let processCommand = async (msg, params, tiers) => {
  try {
    ...
    await DB_UTILS.setField(msg.author.id, "last_social", new Date());
    let score = await DB_UTILS.getField(msg.author.id, "total_score");
    msg.reply(SCORE + " point added successfully for today's social!\nYour score is now "+score);
  } catch (e) {
...
let commandValidator = async (msg, params, tiers) => {
  ...
  try {
    let last_social = await DB_UTILS.getField(msg.author.id, "last_social");
    if (last_social) {
      let UTC_DATE = last_social.getUTCDate();
      let UTC_DATE_CUR = new Date().getUTCDate();
      if (UTC_DATE === UTC_DATE_CUR) {
        // command invalid because two commands in same day not allowed
        throw new Error("You have already gained social points for today. Wait for next day(in UTC)");
      }
    }
    return true;
  } catch (e) {
